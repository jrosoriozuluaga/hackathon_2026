...
description: >-
  Generate Harness v0 Pipeline YAML for CI/CD workflows as local resource artifacts. Supports CI stages
...
Generate Harness v0 Pipeline YAML as local resource artifacts.
...
5. **Package the local artifact** — First verify the project context exists in user-provided context, local inventory, or generated project artifacts, then record the pipeline YAML as a `yamlPipeline` artifact string.
...
After generating the YAML, package it locally for review or later application:

1. **Verify the project context exists** — Check local inventory, generated project artifacts, or explicit user confirmation for `org_id` and `project_id`.
2. **Package the pipeline** — Store the pipeline YAML serialized as a **`yamlPipeline`** string. Do not use a nested JSON `pipeline` object when preparing an apply payload.
...
resource_id: <pipeline_identifier>
checks:
  - YAML has pipeline root key
  - org/project identifiers match scope
  - referenced resources are listed in local inventory or confirmed by user
```
...
- Verify the target project exists in local inventory or user-provided context before packaging the pipeline artifact.
...
- **Project not found** — Verify the project exists in local inventory, generated project artifacts, or user-provided context; otherwise ask for the correct org/project values.
...
- `DUPLICATE_IDENTIFIER` — Pipeline already exists; prepare an update artifact instead
- `CONNECTOR_NOT_FOUND` — Create the connector first or fix connectorRef
- `ACCESS_DENIED` — Check the applying user's Harness permissions
...
- Connector auth expired - include a post-apply connector connection test
- Delegate offline - ask the user for current delegate status or a Harness export
