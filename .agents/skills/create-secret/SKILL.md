...
name: create-secret
description: >-
  Generate Harness Secret definitions and manage secrets as local resource artifacts. Supports SecretText,
...
Generate Harness Secret definitions and manage secrets as local resource artifacts.

## Local Artifact Operations

| Operation | Resource Type | Purpose |
|-----------|--------------|---------|
| Check local inventory | `secret` | Find existing secret identifiers without exposing values |
| Draft artifact | `secret` | Create a local secret definition |
| Update artifact | `secret` | Revise metadata, scope, or manager references |
| Mark for removal | `secret` | Document a deletion request for external review |
| Validate shape | `secret` | Check required fields using local references |
...
| Scope | Visibility | Local Scope Fields |
|-------|-----------|----------------|
...
scope: <org>/<project>
checks:
  - identifier is unique in local inventory
  - scope prefix is documented
  - value is not exposed in logs or changelog
```
...
1. Verify the secret manager connector is healthy using local inventory, user-provided export, or a post-apply Harness UI check
