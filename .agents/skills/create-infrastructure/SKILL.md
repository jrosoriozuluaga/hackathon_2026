...
description: Generate Harness Infrastructure Definition YAML for deployment targets as local resource artifacts. Use when user says "create infrastructure", "infrastructure definition", "k8s cluster config", "deployment target", or wants to configure where workloads run.
metadata:
  author: Harness
  version: 2.0.0
license: Apache-2.0
...
Generate Harness Infrastructure Definition YAML as local resource artifacts.
...
4. **Record the infrastructure artifact locally** with org/project scope, environment reference, deployment type, and YAML body.
