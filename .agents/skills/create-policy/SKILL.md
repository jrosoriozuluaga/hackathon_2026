...
name: create-policy
description: >-
  Create OPA governance policies for Harness as local resource artifacts. Define policies that enforce compliance rules on
...
Create OPA governance policies for Harness Software Supply Chain Assurance (SCS) as local resource artifacts.
...
### Step 2: Create the Policy Artifact

```
resource_type: policy
org_id: <organization>
project_id: <project>
artifact: <policy definition>
```

OPA policies should be tracked locally with their policy set, enforcement mode, target scope, and Rego source.

### Step 3: Verify Compliance Results

After a policy artifact is generated, document how compliance should be checked on artifacts or repositories after the user applies it in Harness:

```
resource_type: scs_compliance_result
org_id: <organization>
project_id: <project>
source: user-provided export or Harness UI after policy application
```
