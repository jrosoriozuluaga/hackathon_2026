...
description: Generate Harness Connector YAML for integrations as local resource artifacts. Use when user says "create connector", "git connector", "aws connector", "docker connector", "cloud connector", or wants to connect Harness to external services.
metadata:
  author: Harness
  version: 2.0.0
license: Apache-2.0
...
Generate Harness Connector YAML as local resource artifacts.
...
3. **Record the connector artifact locally** with org/project scope, identifier, name, type, and YAML body.
4. **Document validation steps** the user should run in Harness after applying the connector, including connection testing.
...
org_id: <organization>
project_id: <project>
artifact: <connector YAML>
```

Document post-apply test:
```
resource_type: connector
action: test_connection
resource_id: <connector_identifier>
expected_result: connection succeeds in Harness after artifact is applied
```

Use local references or the connector examples above to choose the connector type. If the required type is missing, add a reference note before generating the artifact.
```
resource_type: connector_catalogue
source: local skill references or user-provided Harness inventory
```
...
- Always include a post-apply connection test instruction.
